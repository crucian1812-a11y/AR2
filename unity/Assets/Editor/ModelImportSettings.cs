...
public class ModelImportSettings : AssetPostprocessor
{
    ...
    private void OnPreprocessAnimation()
    {
        ...
        for (int i = 0; i < clips.Length; i++)
        {
            ...
            // Настройки запекания корневого движения (lockRootRotation,
            // keepOriginalPositionY) здесь сознательно НЕ трогаются.
            //
            // Они писались, когда риг планировался как Humanoid. С Generic
            // положение и поворот арматуры — это не «корневое движение», а
            // обычные кривые трансформа, и именно ими бойцы расставлены
            // друг относительно друга: верхний сидит на нижнем, потому что
            // его таз поднят и развёрнут. Запекание корня вырезало бы эти
            // кривые, и оба бойца сошлись бы в одну точку в позе покоя —
            // ровно та поломка, от которой эти настройки должны были
            // защищать.
            //
            // За то, чтобы никто не уползал с татами, отвечает другое:
            // корневые объекты обоих бойцов в игре стоят в центре и не
            // двигаются, а всё остальное живёт внутри клипа.
        }
