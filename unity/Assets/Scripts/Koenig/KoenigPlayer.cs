...
namespace Koenig
{
    ...
    public class KoenigPlayer : MonoBehaviour
    {
        ...
        // Наклон камеры от горизонта. Пятьдесят градусов оказались вдвое
        // круче нужного: при них вертикальная стена в 7 м проецируется на
        // экран короче (4.5 м), чем скат кровли (3.8 м), — в кадре одни
        // крыши и макушки, а улицы нет. Проверено рендером той же сцены.
        //
        // У Diablo 2 угол около тридцати. При 32° стена даёт 5.9 м экрана
        // против 2.6 м у ската: фасады с окнами и дверями выходят вперёд,
        // и герой заодно вырастает с 9% до 12% высоты кадра.
        private const float CamPitch = 32f;
        ...
        //
        // ПОЧЕМУ 22.5, А НЕ 40. Ближняя плоскость при ортографии режет всё,
        // что оказалось «за» камерой по оси взгляда, и высокий дом далеко
        // позади героя уходит за неё, оставляя в кадре прямой вертикальный
        // обрез. Условие «не режет» считается точно:
        //
        //     1.179 · CamBack  ≥  1 + 0.53 · h + 0.848 · d
        //
        // где h — высота геометрии, d — насколько она южнее героя. При
        // CamBack = 36 дом высотой 12 м не режется вплоть до 41 м позади
        // героя, а дальше он всё равно за краем кадра.
        //
        // Сорок метров условию тоже удовлетворяли, но платили за это
        // тенями: камера оказывалась в 75 м от героя, а дальность теней
        // URP — 90 м, и три каскада растягивались на всю эту длину. Тени
        // выходили мылом. При 22.5/36 расстояние до героя 42 м, каскады
        // ложатся вчетверо плотнее — и именно тени держат объём улицы.
        //
        ...
        // наложение силуэтов.
        private const float CamHeight = 22.5f;
        private const float CamBack = 36f;     // ≈ 22.5 / tan(32°), ось смотрит в героя
        private const float CamFollow = 6f;

        // Половина вертикали кадра в метрах. 6.6 даёт 13.2 м по вертикали:
        // герой ростом 1.85 м занимает на экране 1.57 м, то есть восьмую
        // часть высоты кадра — посадка Diablo 2. По горизонтали кадр берёт
        // от 23 м (16:9) до 29 м (20:9), и под эту ширину подогнана улица:
        // мостовая 11 м, фасады по краям, свес кровли над мостовой.
        ...
        private void BuildCamera()
        {
            ...
            _cam.farClipPlane = 200f;   // до горизонта моря
            // Пене прибоя нужна глубина сцены: она сравнивает поверхность
            // воды с песком под ней и сама находит линию берега.
