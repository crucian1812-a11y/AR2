...
namespace Koenig
{
    ...
    public class KoenigPlayer : MonoBehaviour
    {
        ...
        // мельчал вдвое.
        //
        // При ортографии высота камеры НЕ ВЛИЯЕТ на картинку вообще:
        // размер задаёт только OrthoSize. Поэтому камера уходит на 22 м,
        // где над самым высоким домом двенадцать метров запаса, а кадр
        // остаётся тем же. Попасть внутрь геометрии она больше не может
        // в принципе — не «маловероятно», а по устройству.
        //
        // Плата: пропадает перспектива, дальний дом ровно того же размера,
        // что ближний. Это честная изометрия Diablo 2; глубину теперь
        // держат туман, разная этажность и наложение силуэтов.
        private const float CamHeight = 22f;
        private const float CamBack = 18.5f;   // = 22 / tan(50°), ось смотрит в героя
        private const float CamFollow = 6f;

        // Половина вертикали кадра в метрах. 5.5 даёт 11 м по вертикали:
        // пачка врагов (радиусом 1.8 м) и босс 4.3 м помещаются с запасом
        // на отход, а герой занимает около десятой части высоты — посадка
        // Diablo 2. Ставили 7 — герой оставался таким же мелким, как был.
        private const float OrthoSize = 5.5f;
        ...
        private void BuildCamera()
        {
            ...
            // Камера ушла на 22 м, поэтому ближняя плоскость не может
            // оставаться на 0.1 — иначе точность буфера глубины тратится
            // на пустой воздух над крышами.
