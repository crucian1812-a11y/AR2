...
public class DesertWorld : WorldBuilder
{
    ...
    protected override void Build()
    {
        ...
        // И подножие самой высокой столовой горы. Гора ставится через
        // OnGround, а примыкающая к ней верхняя площадка подъёма и босс
        // заданы абсолютными высотами: сходятся они, только если земля
        // там ровно на нуле. Точка лежит вне общей ровной площадки при
        // амплитуде рельефа 11, так что нулём её надо сделать явно.
        FlattenArea(-52f, -18f, 20f, 12f);
        ...
        Reserve(OnGround(56f, -60f, 2.1f), 14f);
        Reserve(OnGround(-14f, 20f, 4f), 7f);
        BuildDunes();
