...
public struct Opponent
{
    ...
    // Внешность. Двадцать соперников в одинаковых телах читались бы как
    // один и тот же человек в разных настроениях; тон кожи и рост стоят
    // ноль и делают из списка зал, полный разных людей.
    public Color Skin;
    public float Build;      // множитель роста, около единицы
}
...
public static class Career
{
    ...
    // Тона кожи. Умножаются на запечённое альбедо, поэтому лицо, брови и
    // щетина остаются на месте — меняется только тон.
    private static readonly Color[] Skins =
    {
        new Color(1.00f, 0.98f, 0.96f),
        new Color(0.93f, 0.85f, 0.77f),
        new Color(0.79f, 0.66f, 0.55f),
        new Color(0.61f, 0.48f, 0.40f)
    };

    /// Прибавка к шансам игрока за собственный пояс. Небольшая: пояс
    /// должен ощущаться, но не отменять борьбу.
    public static float Edge
    {
        get
        {
            Load();
            return (int)Belt * 0.022f + Stripes * 0.004f;
        }
    }

    /// Внешность игрока — та же система, но постоянная.
    public static Color PlayerSkin { get { return Skins[1]; } }
    ...
    public static Opponent Next()
    {
        ...
        o.Skin = Skins[(index * 7 + 3) % Skins.Length];
        // Рост в пределах трёх процентов: больше — и парная анимация
        // начнёт расходиться, руки перестанут попадать в захват.
        o.Build = 0.97f + ((index * 5) % 7) * 0.01f;
