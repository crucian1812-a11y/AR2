...
public static class Heroes
{
    ...
    // Цены посчитаны от того, сколько монет реально лежит в мирах
    // (около 230 за прохождение). Прежняя лестница до 400 за героя
    // требовала 5500 монет — витрина была декоративной.
    public static int Price(int index)
    {
        index = Clamp(index);
        if (index < FreeChars) return 0;
        return 12 + (index - FreeChars) * 3;
    }
